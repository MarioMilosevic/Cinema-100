    ...
    <div className="w-[300px] flex flex-col">
      <div className="w-full h-[380px] relative">
        ...
        <div className="bg-gray-900 absolute top-0 right-0 w-full h-full transition-all duration-300 opacity-0 hover:opacity-70">
          <FaBookmark
            className={`absolute top-2 right-2 cursor-pointer w-5 h-5 ${isBookmarked ? 'text-orange-500' : 'text-gray-100'}`}
            onClick={() => setIsBookmarked((prev) => !prev)}
          />
          <div className="flex items-center gap-2 z-10 absolute bottom-1/2 right-1/2 translate-x-1/2 translate-y-1/2 rounded-full bg-gray-700 text-gray-100 px-4 py-2 cursor-pointer hover:text-gray-900 hover:bg-white active:bg-orange-700 active:text-gray-100">
            <span>See more</span>
            <FaSearch />
          </div>
        </div>
